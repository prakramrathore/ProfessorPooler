...
if st.button('Search'):
    ...
    if len(result) == 0:
        ...
        for i in result:
            ...
            target = target.iloc[:40, [0,2]]

            total_score = target.iloc[:, 1].sum()
            res = []
            while(len(res) < 10):
                
                # calculate the probability of each professor
                probabilities = target.iloc[:, 1] / total_score
                
                # randomly select a professor based on the probability
                idx = random.choices(range(len(target)), weights=probabilities)[0]

                if idx in target.index:
                    # get the name of the professor
                    name = target.iloc[idx, 0]

                    res.append(name)

                    target = target.drop(index=idx)
